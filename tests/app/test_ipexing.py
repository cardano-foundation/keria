...
def test_multisig(seeder, helpers):
    with (helpers.openKeria(salter=core.Salter(raw=b'0123456789abcM00')) as (agency0, agent0, app0, client0), \
            ...
            helpers.openKeria(salter=core.Salter(raw=b'0123456789abcM04')) as (vagency0, vagent0, vapp0, vclient0), \
            helpers.openKeria(salter=core.Salter(raw=b'0123456789abcM05')) as (vagency1, vagent1, vapp1, vclient1)):
        ...
        serder = eventing.incept(keys=vkeys,
                                 isith="2",
                                 nsith="2",
                                 ndigs=ndigs,
                                 code=coring.MtrDex.Blake3_256,
        ...
        sigers = [verifierSigner0.sign(ser=serder.raw, index=0).qb64, verifierSigner1.sign(ser=serder.raw, index=1).qb64]
        ...
        while not vagent0.counselor.complete(prefixer=coring.Prefixer(qb64=serder.pre), seqner=coring.Seqner(sn=0)):
            doist.recur(deeds=deeds)

        assert vagent1.counselor.complete(prefixer=coring.Prefixer(qb64=serder.pre), seqner=coring.Seqner(sn=0)) is True
        ...
        for name, agent in [("verifier", vagent0), ("verifierParticipant0", vagent0), ("verifierParticipant1", vagent1)]:
        ...
        applySerder, end = exchanging.exchange(route="/ipex/apply",
                                               payload={'m': 'Applying for a credential', 's': schema, 'a': {'LEI': '254900DA0GOGCFVWB618'}},
                                               sender=verifierPre,
                                               embeds=dict(),
                                               recipient=holderPre,
                                               date=helping.nowIso8601())

        applySigers = [verifierSigner0.sign(ser=applySerder.raw, index=0),
        ...
        offerSerder, end = exchanging.exchange(route="/ipex/offer",
        ...
        offerSigers = [holderSigner0.sign(ser=offerSerder.raw, index=0),
        ...
        agreeSerder, end = exchanging.exchange(route="/ipex/agree",
                                               payload={'m': 'OK!'},
                                               sender=verifierPre,
                                               embeds=dict(),
                                               recipient=holderPre,
        ...
        agreeSigers = [verifierSigner0.sign(ser=agreeSerder.raw, index=0),
        ...
        grantSigers = [holderSigner0.sign(ser=grantSerder.raw, index=0),
        ...
        admitSigers = [verifierSigner0.sign(ser=admitSerder.raw, index=0),
...
def test_ipex_agree(helpers, mockHelpingNowIso8601):
    with helpers.openKeria() as (_, agent, app, client):
        ...
        agreeSerder, end = exchanging.exchange(route="/ipex/agree",
                                                ...
                                                recipient=pre1,
                                                date=helping.nowIso8601())
        assert agreeSerder.ked == {'a': {'i': 'EFnYGvF_ENKJ_4PGsWsvfd_R6m5cN-3KYsz_0mAuNpCm', 'm': 'Agreed'},
