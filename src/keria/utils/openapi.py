...
def createDataclassNestedField(
    key: str, customType: type, isOptional: bool
    ...
    return createOptionalField(
        key, customType, mm_fields.Nested, (nestedSchema,), {}, isOptional
    )
...
def createCustomNestedField(
    ...
    if hasattr(customType, "__origin__") and customType.__origin__ is Union:
        ...
        if len(union_args) == 2 and str in union_args:
            ...
            if (
                list_type
                and len(list_type.__args__) == 1
                and list_type.__args__[0] is str
            ):
                return createOptionalField(
                    key, customType, StringOrStringArrayField, (), {}, isOptional
                )

        # For other Union types, fall back to Raw field with anyOf
        return createOptionalField(
            key, customType, UnionField, (customType,), {}, isOptional
        )
...
def createRegularField(
    key: str, value: Any, isOptional: bool = True
...
def dataclassFromFielddom(
    name: str,
    fieldDom: serdering.FieldDom,
    customTypes: Optional[Dict[str, type]] = None,
):
    """
...
def applyAltConstraintsToOpenApiSchema(
    openApiSchemaDict: dict, altConstraints: Dict[str, str]
):
    """
